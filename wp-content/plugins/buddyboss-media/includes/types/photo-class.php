	...
	class BuddyBoss_Media_Type_Photo extends BP_Component {
		...
		public function setup_actions() {
			...
			if ( ( ! isset( $bp->groups->current_group->id ) ) ||
				...
				add_filter( 'bp_activity_allowed_tags', array( $this, 'bp_activity_allowed_tags' ) );
			} else {
		...
		public function bp_activity_allowed_tags( $activity_allowedtags ) {
		    if ( ! empty( $activity_allowedtags['a'] ) ) {
		...
		public function minified_assets() {
			...
			// JS > PhotoSwipe
			wp_enqueue_script( 'buddyboss-media-popup', $assets . '/vendor/photoswipe/photoswipe' . $suffix . '.js', array( 'jquery' ), '4.1.3', false );
			wp_enqueue_script( 'buddyboss-media-popup-ui-default', $assets . '/vendor/photoswipe/photoswipe-ui-default' . $suffix . '.js', array( 'jquery' ), '4.1.3', false );
			wp_enqueue_style( 'buddyboss-media-popup-ui-ps-css', $assets . '/vendor/photoswipe/photoswipe.css' );
			wp_enqueue_style( 'buddyboss-media-popup-ui-default-css', $assets . '/vendor/photoswipe/default-skin/default-skin.css' );
			...
			if ( ! in_array( bp_current_action(), $bp_upload_actions ) ) {
				...
				wp_enqueue_script( 'moxie', $assets . '/vendor/plupload2/moxie.min.js', array( 'jquery' ), '1.5.7' );
				wp_enqueue_script( 'plupload', $assets . '/vendor/plupload2/plupload.min.js', array( 'jquery', 'moxie' ), '2.3.6' );
			}
			...
			// Fancybox
			wp_enqueue_script( 'jquery-fancybox', $assets . '/vendor/fancybox/jquery.fancybox' . $suffix . '.js', array( 'jquery' ), '3.5.7', true );
			wp_enqueue_style( 'jquery-fancybox', $assets . '/vendor/fancybox/jquery.fancybox' . $suffix . '.css', array(), '3.5.7', 'all' );
				...
				<div id="buddyboss-media-add-photo">
					...
					<!-- Fake add photo button will be clicked from js -->
                                        <?php if( ('yes' === $show_uploadbox) && $iPod || $iPhone || $iPad ) { ?>
                                            <button class="buddyboss-activity-media-add-photo-button" id="buddyboss-media-open-uploader-button" data-fancybox data-src="#buddyboss-media-bulk-uploader-wrapper" href="javascript:;"></button><?php
                                        } else { ?>
                                            <button class="buddyboss-activity-media-add-photo-button" id="buddyboss-media-open-uploader-button" data-fancybox data-src="#buddyboss-media-bulk-uploader-wrapper" href="javascript:;" style="<?php echo ( ! empty( $show_uploadbox ) ) ? 'display:none;' : ''; ?>"></button>
