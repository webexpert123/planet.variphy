	...
	public function script_templates() {
		...
		<script type="text/html" id="buddyboss-bbpress-media-tpl-add-photo">
			<div class="buddyboss-bbpress-media-add-photo">
				...
				<a class="open-uploader-button buddyboss-bbpress-media-add-photo-button button submit" data-fancybox data-src="#buddyboss-bbpress-media-bulk-uploader-wrapper" href="javascript:;" style="<?php echo ( ! empty( $show_uploadbox ) ) ? 'display:none;' : ''; ?>"></a>
...
//					);
			...
			foreach ( $attachments as $attachment ) {
				...
				if ( ! empty( $image ) && is_array( $image ) && count( $image ) > 2 ) {
					...
					$owner = ( $author_id == get_current_user_id() ) ? '1' : '0';

					if ( $full !== false && is_array( $full ) && count( $full ) > 2 ) {

						$filesize = isset( $filenames[$img_counter] ) ? $filenames[$img_counter] :  $filenames[0];

						$all_imgs_html .= '<a data-activity-id="' . $act_id . '" data-width="' . esc_attr( $w ) . '" data-height="' . esc_attr( $h ) . '" data-id="' . esc_attr( $media_id ) . '" class="buddyboss-media-photo-wrap size-' . $filesize . '" ' . $height_markup . '  href="' . $full[0] . '" ' . $maybe_display_none . '>';
						...
						$all_imgs_html .= '<img data-owner="'.$owner.'" data-photo-id="' . $media_id . '" class="buddyboss-media-photo" src="' . $src . '"' . $width_markup . ' data-comment-count="' . $comment_count . '" data-permalink="'. $reply .'"
						 data-media="'. $act_id .'" data-favorite-count="'.$favorite_count.'" data-bbmfav="'.$data_fav.'" data-comment-count="'.$comment_count.'" href="" /></a>';
					} else {
						$filesize = isset( $filenames[$img_counter] ) ? $filenames[$img_counter] :  $filenames[0];

						$all_imgs_html .= '<img data-owner="'.$owner.'" ' . $maybe_display_none . ' data-photo-id="' . $media_id . '"  data-comment-count="' . $comment_count . '"  class="buddyboss-media-photo size-' . $filesize . '" src="' . $src . '"' . $width_markup . '
