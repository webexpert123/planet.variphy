...
class ANON_ZAPIERWEBHOOKS {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'support_link'        => $uncanny_automator->get_author_support_link( $this->trigger_code, 'integration/zapier/' ),
			'is_pro'              => true,
			'integration'         => self::$integration,
			'code'                => $this->trigger_code,
			/* translators: Anonymous trigger - WP Zapier */
			...
			'action'              => 'uncanny_automator_pro_zapier_webhook',
			'priority'            => 10,
			'accepted_args'       => 2,
			'type'                => 'anonymous',
			...
			'options_group'       => [
				'WEBHOOK_DATA' => [
					[
						...
						'default_value'     => '',
						'is_ajax'           => true,
						'endpoint'          => 'webhook_url_ANON_ZAPIERWEBHOOKS',
					],
					[
	...
	public function get_samples_js() {
        ...
        <script>
            ...
            function ($button, data, modules) {
                ...
                let dataToBeSent = {
                    action: 'get_samples_ANON_ZAPIERWEBHOOKS',
                                ...
                                if (foundResults) {
									...
									*/
                                    ...
                                    jQuery.each(rows, function (index, row) {
                                        // Create row
                                        let $row = jQuery('<tr><td class="ANON-ZAPIERWEBHOOKS-sample-table-td-key">' + row.key + '</td><td>' + UncannyAutomator.i18n.tokens.tokenType[row.type] + '</td><td class="ANON-ZAPIERWEBHOOKS-sample-table-td-data">' + row.data + '</td></tr>');
	...
	public function inline_css() {
        ...
        <style>
            ...
            .ANON-ZAPIERWEBHOOKS-sample-table-td-key {
                color: #1b92e5 !important;
                font-weight: 500 !important;
            }

            .ANON-ZAPIERWEBHOOKS-sample-table-td-data {
			...
			foreach ( $recipes as $recipe ) {
				foreach ( $recipe['triggers'] as $trigger ) {
					if ( key_exists( 'WEBHOOK_URL', $trigger['meta'] ) && false !== strpos( $trigger['meta']['WEBHOOK_URL'], $route ) ) {
						...
						$available_hooks[] = $_hooks;
						do_action( 'uncanny_automator_pro_zapier_webhook', $_hooks, $recipe );
					}
				}
			}
		}
