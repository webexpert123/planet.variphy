...
class UOA_ANON_WEBHOOKS {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'support_link'        => $uncanny_automator->get_author_support_link( $this->trigger_code, 'knowledge-base/webhook-triggers/' ),
			'is_pro'              => true,
			...
			/* translators: Anonymous trigger - WordPress Core */
			'action'              => 'uncanny_automator_pro_wp_webhook',
			'priority'            => 10,
			'accepted_args'       => 2,
			'type'                => 'anonymous',
			...
			'options_group'       => [
				'WEBHOOK_DATA' => [
					[
						...
						'default_value'     => '',
						'is_ajax'           => true,
						'endpoint'          => 'webhook_url_UOA_ANON_WEBHOOKS',
					],
					// Header
					[
						'input_type' => 'repeater',
        ...
        <script>
            ...
            function ($button, data, modules) {
                ...
                let config = {
                    ...
                    // Links
                    links: {
                        noResultsSupport: "<?php echo Utilities::utm_parameters( 'https://automatorplugin.com/knowledge-base/webhook-triggers/', 'no_samples', 'get_help_link' ); ?>",
                    },
                    // i18n
                    i18n: {
                        ...
                        samplesModalWarning: "<?php printf( /* translators: 1. Button */
							__( 'Clicking on \"%1$s\" will remove your current fields and will use the ones on the table above instead.', 'uncanny-automator-pro' ), '{{confirmButton}}' ); ?>",
                ...
                let dataToBeSent = {
                    action: 'get_samples_UOA_ANON_WEBHOOKS',
                                ...
                                if (foundResults) {
									...
									*/
                                    ...
                                    jQuery.each(rows, function (index, row) {
                                        // Create row
                                        let $row = jQuery('<tr><td class="ANON-WEBHOOKS-sample-table-td-key">' + row.key + '</td><td>' + UncannyAutomator.i18n.tokens.tokenType[row.type] + '</td><td class="ANON-WEBHOOKS-sample-table-td-data">' + row.data + '</td></tr>');
                                    ...
                                    modal.setEvents({
                                        onConfirm: function () {
                                            ...
                                            let webhookFields = data.item.options.WEBHOOK_DATA.fields[2];
	...
	public function inline_css() {
        ...
        <style>
            ...
            .ANON-WEBHOOKS-sample-table-td-key {
                color: #1b92e5 !important;
                font-weight: 500 !important;
            }

            .ANON-WEBHOOKS-sample-table-td-data {
	...
	public function init_rest_api_samples() {
		...
		foreach ( $results as $result ) {
			if ( ! empty( $result->value ) ) {
				...
				$created_at    = get_option( $expiry_option, current_time( 'U' ) );
				$difference    = round( abs( current_time( 'U' ) - $created_at ) / 60, 2 );
			...
			foreach ( $recipes as $recipe ) {
				foreach ( $recipe['triggers'] as $trigger ) {
					if ( key_exists( 'WEBHOOK_URL', $trigger['meta'] ) && false !== strpos( $trigger['meta']['WEBHOOK_URL'], $route ) ) {
						...
						$available_hooks[] = $_hooks;
						do_action( 'uncanny_automator_pro_wp_webhook', $_hooks, $recipe );
					}
				}
			}
		}
