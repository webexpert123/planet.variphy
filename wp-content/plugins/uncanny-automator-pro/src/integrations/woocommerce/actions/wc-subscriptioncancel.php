...
class WC_SUBSCRIPTIONCANCEL {
	...
	public function define_action() {
		...
		$action = array(
			...
			'sentence'           => sprintf( esc_attr__( "Cancel the user's subscription to {{a subscription product:%1\$s}}", 'uncanny-automator-pro' ), $this->action_meta ),
			/* translators: Action - WooCommerce */
			'select_option_name' => esc_attr__( "Cancel the user's subscription to {{a subscription product}}", 'uncanny-automator-pro' ),
			'priority'           => 99,
			'accepted_args'      => 1,
			'execution_function' => array( $this, 'wcs_cancel_user_subscription' ),
			'options'            => array(
				Automator()->helpers->recipe->woocommerce->options->pro->all_wc_subscriptions(null, $this->action_meta, false),
			),
		);
