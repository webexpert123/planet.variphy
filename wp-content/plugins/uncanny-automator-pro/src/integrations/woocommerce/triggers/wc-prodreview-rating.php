...
class WC_PRODREVIEW_RATING {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'options'             => array( 
				$uncanny_automator->helpers->recipe->woocommerce->options->all_wc_products( __( 'Product', 'uncanny-automator' ) , $this->trigger_meta ),
				$uncanny_automator->helpers->recipe->field->select(
					array(
	...
	public function wc_product_reviewed( $comment_id, $comment_approved, $commentdata ) {
		...
		foreach ( $recipes as $recipe_id => $recipe ) {
			foreach ( $recipe['triggers'] as $trigger ) {
				...
				Utilities::log( [ $required_post[ $recipe_id ][ $trigger_id ] => $comment->comment_post_ID, $required_condition[ $recipe_id ][ $trigger_id ] => 'condition', $required_rating[ $recipe_id ][ $trigger_id ] => $rating ], 'check post', true, 'wc-log' );
