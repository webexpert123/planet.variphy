...
class WC_PURCHVARIABLEPROD {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( __( 'A user purchases {{A variable product:%1$s}} with {{a variation:%2$s}} selected', 'uncanny-automator-pro' ),
				'WOOVARIABLEPRODUCTS' . ':' . $this->trigger_meta, $this->trigger_meta ),
			/* translators: Logged-in trigger - WooCommerce */
			...
			'options_group'       => [
				$this->trigger_meta => [
					$uncanny_automator->helpers->recipe->woocommerce->options->pro->all_wc_variable_products(
