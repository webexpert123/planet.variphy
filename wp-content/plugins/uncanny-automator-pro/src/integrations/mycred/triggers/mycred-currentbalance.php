...
class MYCRED_CURRENTBALANCE {
		...
		$trigger = array(
			...
			'options'             => [
				array(
						...
						'label'           => __( 'Balance amount', 'uncanny-automator-pro' ),
						'supports_tokens' => true,
						'required'        => true,
						'placeholder' => esc_attr__( 'Example: 1.1', 'uncanny-automator' ),
					),
				$uncanny_automator->helpers->recipe->mycred->options->list_mycred_points_types( esc_attr__( 'Points type', 'uncanny-automator-pro' ), $this->trigger_meta ),
			],
		);
	...
	public function mycred_current_balance( $user_id, $current_balance, $amount, $point_type ) {
		...
		$required_point_type = $uncanny_automator->get->meta_from_recipes( $recipes, $this->trigger_meta );
		$required_balance    = $uncanny_automator->get->meta_from_recipes( $recipes, 'CURRENTBALANCE' );
		$matched_recipe_ids  = [];
