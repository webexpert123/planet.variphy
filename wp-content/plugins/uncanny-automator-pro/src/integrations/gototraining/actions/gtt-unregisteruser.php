...
class GTT_UNREGISTERUSER {
	...
	public function define_action() {
		...
		$action = array(
			...
			'sentence'           => sprintf( __( 'Remove the user from {{a training session:%1$s}}', 'uncanny-automator-pro' ), $this->action_meta ),
			'select_option_name' => __( 'Remove the user from {{a training session}}', 'uncanny-automator-pro' ),
			'priority'           => 10,
			'accepted_args'      => 1,
	...
	public function gtt_unregister_user( $user_id, $action_data, $recipe_id, $args ) {
		...
		if ( empty( $user_registrant_key ) ) {
			$error_msg                           = __( 'User was not registered for training session.', 'uncanny-automator-pro' );
			$action_data['do-nothing']           = true;
			$action_data['complete_with_errors'] = true;
			$uncanny_automator->complete_action( $user_id, $action_data, $recipe_id, $error_msg );
		...
		$result = $uncanny_automator->helpers->recipe->gototraining->pro->gtt_unregister_user( $user_id, $training_key );
		if ( ! $result['result'] ) {
