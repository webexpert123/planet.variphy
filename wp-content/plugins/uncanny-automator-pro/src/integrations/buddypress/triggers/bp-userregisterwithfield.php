...
class BP_USERREGISTERWITHFIELD {
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( __( 'A user registers with {{a specific value:%1$s}} in {{a specific field:%2$s}}', 'uncanny-automator-pro' ), $this->trigger_meta, 'SUBVALUE:' . $this->trigger_meta ),
			/* translators: Logged-in trigger - BuddyPress */
			...
			'options_group'       => [
				$this->trigger_meta => [
					$uncanny_automator->helpers->recipe->buddypress->pro->list_base_profile_fields( null, $this->trigger_meta ),
