...
class GTW_REGISTERUSER {
	...
	public function define_action() {
		...
		$action = array(
			...
			'sentence'           => sprintf( __( 'Add the user to {{a webinar:%1$s}}', 'uncanny-automator-pro' ), $this->action_meta ),
			'select_option_name' => __( 'Add the user to {{a webinar}}', 'uncanny-automator-pro' ),
			'priority'           => 10,
			'accepted_args'      => 1,
	...
	public function gtw_register_user( $user_id, $action_data, $recipe_id, $args ) {
		...
		$result = $uncanny_automator->helpers->recipe->gotowebinar->pro->gtw_register_user( $user_id, $webinar_key );
		if ( ! $result['result'] ) {
