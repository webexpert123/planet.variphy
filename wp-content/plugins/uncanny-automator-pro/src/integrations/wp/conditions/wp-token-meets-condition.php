...
class WP_TOKEN_MEETS_CONDITION extends Action_Condition {
	...
	public function fields() {
		...
		return array(
			...
			Automator()->helpers->recipe->field->text(
				array(
					...
					/* translators: 1. Learn more */
						...
						// Anchor
						sprintf(
							'<a href="%1$s" target="_blank">%2$s <automator-icon id="external-link"></automator-icon></a>',
			...
			// Criteria field
			Automator()->helpers->recipe->field->select_field_args( $conditions ),
			// Value field
			Automator()->helpers->recipe->field->text(
				array(
					'option_code' => 'VALUE',
	...
	public function evaluate_condition() {
		$parsed_token = $this->get_parsed_field( 'TOKEN' );
		$parsed_value = $this->get_parsed_field( 'VALUE' );
		...
		$criteria     = $this->get_parsed_field( 'CRITERIA' );
		$result       = $this->check_logic( $parsed_token, $criteria, $parsed_value );
	...
	public function check_logic( $parsed_token, $criteria, $parsed_value ) {
		...
		return apply_filters( 'automator_pro_wp_token_meets_condition_logic_result', $result, $parsed_token, $criteria, $parsed_value );
	}
	...
	public function generate_error_message( $parsed_token, $parsed_value ) {
		...
		$readable_criteria = $this->fields['CRITERIA_readable'];

		$condition_sentence = $parsed_token . '" ' . $readable_criteria . ' "' . $parsed_value . '"';

		return __( 'Failed conditions: "', 'uncanny-automator-pro' ) . $condition_sentence;
	}
}
