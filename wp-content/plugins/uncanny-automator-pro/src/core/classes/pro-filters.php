...
class Pro_Filters {
	...
	public static function activities_filters_html( $tab ) {
		...
		// Get Recipes Name
		if ( $view_exists ) {
			$recipes = $wpdb->get_results( "SELECT DISTINCT(automator_recipe_id) AS id, recipe_title
												FROM {$wpdb->prefix}uap_recipe_logs_view
												ORDER BY recipe_title ASC", ARRAY_A );
		}
		...
		if ( $trigger_view_exists ) {
			if ( $tab == 'trigger-log' ) {
				$triggers = $wpdb->get_results( "SELECT DISTINCT(automator_trigger_id) AS id, trigger_title
													FROM {$wpdb->prefix}uap_trigger_logs_view
													ORDER BY trigger_title ASC", ARRAY_A );
			}
		}
		// Get Actions Name
		...
		if ( $action_view_exists ) {
			if ( $tab == 'action-log' ) {
				$actions = $wpdb->get_results( "SELECT DISTINCT (automator_action_id) AS id, action_title
										FROM {$wpdb->prefix}uap_action_logs_view
										ORDER BY action_title ASC", ARRAY_A );
			}
		}
		...
		if ( $recipe_view_exists ) {
			$users = $wpdb->get_results( "SELECT DISTINCT (user_id) as id, display_name AS title
												FROM {$wpdb->prefix}uap_recipe_logs_view
