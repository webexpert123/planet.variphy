...
class Internal_Triggers_Actions {
	...
	public function boot_triggers_actions_closures() {
		...
		foreach ( $this->active_directories as $dir_name => $object ) {
			...
			$actions_conditions = isset( $this->all_integrations[ $mod ]['conditions'] ) && in_array( 'conditions', $this->directories_to_include[ $mod ], true ) ? $this->all_integrations[ $mod ]['conditions'] : array();
			$vendor   = array();

			$files = array_merge( $tokens, $triggers, $actions, $closures, $actions_conditions, $vendor );
			if ( empty( $files ) ) {
				continue;
			}
			// Loop through all files in directory to create class names from file name
