		...
		if ( ! empty( $errors ) && $validations ) {
			...
			if ( ! empty( $errors ) ) {
				bp_core_add_message( __( 'There was a problem updating some of your profile information. Please try again.', 'buddyboss' ), 'error' );
				    ...
				    global  $wpdb;
					$user_nicename = sanitize_title($_POST['field_25']);

                    $wpdb->update($wpdb->users, array('user_nicename' => $user_nicename), array('ID' => $user_id));

                
                    $link =   trailingslashit( site_url().'/members/' .$user_nicename.'/'. bp_get_profile_slug() . '/edit/group/' . bp_action_variable( 1 ) );
